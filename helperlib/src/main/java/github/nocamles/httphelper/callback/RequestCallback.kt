...
class RequestFourCallback<DataA, DataB, DataC,DataD>(
     ...
     * 当网络请求成功时会调用此方法，随后会先后调用 onSuccessIO、onFinally 方法
     */
    fun onSuccess(block: (dataA: DataA, dataB: DataB, dataC: DataC,dataD:DataD) -> Unit) {
        this.onSuccess = block
    }
     ...
     * 注意外部不要在此处另开子线程，此方法会等到耗时任务完成后再执行 onFinally 方法
     */
    fun onSuccessIO(block: suspend (dataA: DataA, dataB: DataB, dataC: DataC,dataD:DataD) -> Unit) {
        this.onSuccessIO = block
    }
